#!/usr/bin/env python3
"""Fetch and store current MLB first-five-innings odds.

Stores one idempotent row per game/book/line_type in ``mlb.f5_odds`` with F5
moneyline, run line, and total columns when each market is available.

    uv run python scripts/fetch_f5_odds.py --dry-run
"""
...
def f5_rows_by_game_pk(
    *,
    ...
    unmatched = 0
    for odds_row in parse_f5_odds_rows(payload):
...
def _print_rows(rows: Sequence[Mapping[str, object | None]]) -> None:
    for row in rows:
        print(
            ...
            f"{row['bookmaker']} | F5 ML {row.get('away_ml')}/{row.get('home_ml')} "
            f"RL {row.get('away_spread')} {row.get('away_spread_ml')} / "
            f"{row.get('home_spread')} {row.get('home_spread_ml')} | "
            f"total {row.get('total_point')} O{row.get('over_ml')} U{row.get('under_ml')}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out", default="output/odds/f5_odds_current.csv")
...
def main() -> None:
    ...
    if args.odds_json:
        ...
        odds_payload, headers = _fetch_current_f5_odds(
            api_key,
            args.regions,
    ...
    rows, unmatched = f5_rows_by_game_pk(
    ...
    print(
        f"F5 odds {target_date.isoformat()}: slate={len(slate_games)} "
    ...
    if args.dry_run:
        print("dry-run: no F5 odds rows written to DB or CSV")
        return
    if not args.no_db_log:
        db_written = upsert_f5_odds_rows(rows)
        print(f"upserted {db_written} rows into f5_odds")
