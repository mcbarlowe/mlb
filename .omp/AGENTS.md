...
- Outcome model features deliberately exclude `outs` and `runner_on_*` until the pitches table is reloaded with reconstructed state: the legacy columns were dead/mover-only and leaked outcomes (see docs/pitch_outcome_model_plan.md Operational TODOs).
